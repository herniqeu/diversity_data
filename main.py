...
def extract_frames(video_path, output_folder, sample_rate=1, start_time=0, end_time=None, max_frames=None):
    """
    Extract frames from a specified interval of a video file with detailed progress tracking.
    Frames are saved as PNG files in a 'frames' subdirectory within the output folder.
    ...
    """
    # Create the 'frames' subdirectory within the output folder
    ...
    with tqdm(total=frames_to_process, unit='frame', desc="Extracting frames") as pbar:
        while frame_count < frames_to_process:
            ...
            if frame_count % sample_rate == 0:
                ...
                frame_filename = os.path.join(frames_folder, f"{video_name}_{frame_time:.3f}.png")
    ...
    video.release()
    print(f"\nExtracted {saved_count} frames to {frames_folder}")
...
if __name__ == "__main__":
    ...
    saved_frames = extract_frames(video_path, output_folder, sample_rate, start_time, end_time, max_frames)